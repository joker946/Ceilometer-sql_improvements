...
def make_sql_query_from_filter(query, sample_filter,
    ...
    if sample_filter.user:
        sql_where_body += ' AND '.join(['', 'user_id = %s'])
        values.append(sample_filter.user)
    if sample_filter.project:
        sql_where_body += ' AND '.join(['', 'projects_id = %s'])
        values.append(sample_filter.project)
    if sample_filter.resource:
        sql_where_body += ' AND '.join(['', 'resources.resource_id = %s'])
        values.append(sample_filter.resource)
    if sample_filter.message_id:
        sql_where_body += ' AND '.join(['', 'samples.message_id = %s'])
